...
export const MPTStudio: React.FC<MPTStudioProps> = ({ channel, onBack, isEmbedded = false }) => {
  ...
  const [config, setConfig] = useState({
    ...
    heygenAvatarId: channel.mptConfig?.heygenAvatarId || '',
    avatarScale: 1.0, 
    klingModelVersion: channel.defaultKlingModel || 'kling-3.0',
    targetDuration: '60', 
    useStockFootage: channel.defaultVideoType === 'topic',
  ...
  const generateTreatment = async () => {
    ...
    setLoading(true);
    setLog(`🧠 正在呼叫 Agent 導演規劃腳本大綱...`);
    try {
      const res = await fetch('/api/pipeline', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ 
            stage: 'generate_treatment', 
            channel, topic: finalTopic, videoType, productDescription
        })
      });
      const data = await res.json();
      if (data.success) {
        setTreatment(data.treatment);
        setLog(`✅ 企劃書已產出！請在右側審閱並修改。`);
    ...
    try {
      ...
      const res = await fetch('/api/pipeline', {
        ...
        body: JSON.stringify({ 
            ...
            channel, topic: finalTopic, videoType, productDescription, referenceImage, treatment 
        })
      });
      const data = await res.json();
      if (data.success) {
    ...
    <div className={`${isEmbedded ? '' : 'min-h-screen bg-black p-8'} text-white font-sans`}>
      <div className={`${isEmbedded ? '' : 'max-w-6xl mx-auto'} space-y-8`}>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div className="bg-zinc-900/50 p-6 rounded-2xl border border-zinc-800">
              ...
              <div className="mb-4 p-4 bg-zinc-900/30 rounded-xl border border-zinc-800/50">
                  ...
                  <div className="border-t border-zinc-800/80 pt-3 mt-2">
                      ...
                      <textarea value={productDescription} onChange={(e) => setProductDescription(e.target.value)} className="w-full bg-black border border-zinc-700 p-2.5 rounded-lg text-xs text-white outline-none resize-none h-16" />
                  </div>
              </div>
          ...
          {/* ================= 中間：內容審閱 (Treatment & Script) ================= */}
          <div className="lg:col-span-4 space-y-6">
             
            {/* 企劃書區塊 (如果有產出企劃但還沒產出腳本) */}
            {treatment && !script && (
              <div className="bg-zinc-900/80 p-6 rounded-2xl border border-amber-500/50 h-full flex flex-col shadow-[0_0_20px_rgba(245,158,11,0.1)] animate-fade-in">
                ...
                <p className="text-xs text-zinc-400 mb-6">Agent 已為您擬定核心策略。您可以直接在此修改，滿意後再生成分鏡腳本。</p>
                
                <div className="space-y-4 flex-1 overflow-y-auto pr-2 custom-scrollbar">
                    <div>
                        <label className="text-[10px] font-bold text-amber-500 uppercase block mb-1">核心切角 (Core Angle)</label>
                        <textarea value={treatment.coreAngle} onChange={e => setTreatment({...treatment, coreAngle: e.target.value})} className="w-full bg-black/50 border border-amber-900/50 p-3 rounded-lg text-sm text-amber-100 outline-none focus:border-amber-500 resize-none h-20" />
                    </div>
                    <div>
                        <label className="text-[10px] font-bold text-pink-400 uppercase block mb-1">目標受眾情緒 (Target Emotion)</label>
                        <input value={treatment.targetEmotion} onChange={e => setTreatment({...treatment, targetEmotion: e.target.value})} className="w-full bg-black/50 border border-pink-900/50 p-3 rounded-lg text-sm text-pink-100 outline-none focus:border-pink-500" />
                    </div>
                    <div>
                        <label className="text-[10px] font-bold text-blue-400 uppercase block mb-1">前3秒鉤子 (Hook Strategy)</label>
                        <textarea value={treatment.hookStrategy} onChange={e => setTreatment({...treatment, hookStrategy: e.target.value})} className="w-full bg-black/50 border border-blue-900/50 p-3 rounded-lg text-sm text-blue-100 outline-none focus:border-blue-500 resize-none h-20" />
                    </div>
                    <div>
                        <label className="text-[10px] font-bold text-emerald-400 uppercase block mb-1">視覺風格 (Visual Style)</label>
                        <textarea value={treatment.visualStyle} onChange={e => setTreatment({...treatment, visualStyle: e.target.value})} className="w-full bg-black/50 border border-emerald-900/50 p-3 rounded-lg text-sm text-emerald-100 outline-none focus:border-emerald-500 resize-none h-16" />
                    </div>
                </div>
                  ...
                  {script.scenes.map((scene) => (
                    <div key={scene.id} className="p-4 bg-black/40 rounded-xl border border-zinc-800 space-y-3">
                      ...
                      <div>
                          ...
                          <textarea value={scene.narration} onChange={(e) => handleSceneChange(scene.id, 'narration', e.target.value)} className="w-full bg-zinc-900/50 border border-zinc-700/50 p-2 rounded-lg text-sm text-zinc-300 outline-none focus:border-indigo-500 resize-none" rows={2} />
                      </div>
                      <div>
                          <label className="text-[10px] text-emerald-600 font-bold mb-1 flex items-center gap-1">👁️ 畫面提示詞 (Visual Cue)</label>
                          <textarea value={scene.visual_cue} onChange={(e) => handleSceneChange(scene.id, 'visual_cue', e.target.value)} className="w-full bg-emerald-950/20 border border-emerald-900/30 p-2 rounded-lg text-xs text-emerald-400 outline-none focus:border-emerald-500 resize-none" rows={3} />
                      </div>
                    </div>
                  ))}
                </div>
